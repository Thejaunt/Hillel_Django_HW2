from django.db.models import Count, Prefetch, Q, Min
from django.db.models.functions import Coalesce
from django.shortcuts import render, get_object_or_404
from django.core.paginator import Paginator

from .models import Author, Book, BookAuthor, Publisher, Store, StoreBook
...
def publisher_list(request):
    publishers =\
        Publisher.objects.annotate(num_book=Count("book", distinct=True))\
...
def publisher_detail(request, pk):
    obj = get_object_or_404(
        Publisher.objects
        .prefetch_related("book_set", "book_set__authors")
        .annotate(num_book=Count("book", distinct=True), num_auth=Count("book__authors", distinct=True))
        , pk=pk)
...
def book_detail(request, pk):
    obj = get_object_or_404(
        Book.objects
        .prefetch_related("store_set", "authors")
...
def store_detail(request, pk):
    obj = get_object_or_404(Store.objects
                            .prefetch_related("books")
                            .annotate(n_books=Count("books", distinct=True)).order_by("-books__rating", "name")
                            , pk=pk)
